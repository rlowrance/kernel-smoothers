...
function tests.weightedAverage()
   local nObs = 3
   ...
   local kwavg = KernelSmoother(torch.Tensor(nObs, 2), ys, 'epanechnikov quadratic')
   local tol = 1e-6
...
function tests.weights()
   ...
   local kwavg = KernelSmoother(xs, torch.Tensor(nObs), 'epanechnikov quadratic')
